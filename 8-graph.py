...
axis.plot(time, data, color='green', label='V(t)', zorder=1)
axis.legend()
plt.xlabel("Время, c")
...
axis.scatter(new_time, new_data, color='blue', s=7, zorder=2)

charge_time = round(time[data.index(max(data))], 2)
decharge_time = round(time[-1] - charge_time, 2)
plt.text(6, 2.5, "Время зарядки: {} c".format(charge_time))
plt.text(6, 2, "Время разрядки: {} c".format(decharge_time))
